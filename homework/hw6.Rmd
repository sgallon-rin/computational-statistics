...
Recall that the p.d.f. of Exp($\lambda$) is
$$
f(x;\lambda) = \left\{
\begin{aligned}
...
Thus the log likelihood function for estimating $\lambda$ from a sample of size $n$ is
$$
\begin{aligned}
\log L(x_1,...,x_n;\lambda)
&=\sum_{i=1}^n \log f(x;\lambda)\\
&=\sum_{i=1}^n (\log\lambda-\lambda x)\\
&= n\log\lambda - \lambda\sum_{i=1}^nx
\end{aligned}
$$
